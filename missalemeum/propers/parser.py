...
class ProperParser:
    ...
    def proper_exists(self) -> bool:
        return (
            not utils.match(self.proper_id, OBSERVANCES_WITHOUT_OWN_PROPER)
                    ...
                    if re.match(SECTION_REGEX, ln):
                        ...
                        if REFERENCE_REGEX.match(ln):
                            ...
                            if path_bit:
                                ...
                                nested_path: str = (
                                    self._get_full_path(f"{path_bit}.txt", lang) if path_bit else partial_path
                                )
                                if not nested_path:
                                    raise ProperNotFound(f"Proper `{path_bit}.txt` not found.")
                                ...
                                if nested_section is not None:
                                    ...
                                    log.warning(
                                        "Section `%s` referenced from `%s` is missing in `%s`",
                                        nested_section_name,
                                        full_path,
                                        nested_path,
                                    )
    ...
    def _parse_comment(comment: Union[None, Section]) -> dict:
        parsed_comment = {"title": None, "description": "", "rank": None, "additional_info": []}
        if comment is None:
